...
sb.stripplot(x='Plates', y='sedim_thick', hue = 'profile', palette='RdGy', data=df1, jitter=True)
plt.title('sediment thickness', fontsize=10, fontfamily='serif')
plt.legend(bbox_to_anchor=(1.2, .7), loc=1, borderaxespad=-4., ncol=2, 
           markerscale=.4, fancybox=False, shadow=False, mode='expand', title='profiles:',
          title_fontsize=8, labelspacing=.2);
plt.subplots_adjust(bottom=0.15,top=0.85, right=0.80)
#
# subplot 2
ax = fig.add_subplot(222)
sb.stripplot(x='Plates', y='slope_angle', hue = 'profile', palette='PiYG', data=df2, jitter=True)
...
sb.stripplot(x='Plates', y='Mean', hue = 'profile', palette='PRGn', data=df3, jitter=True)
...
sb.stripplot(x='Plates', y='Min', hue = 'profile', palette='RdYlGn', data=df4, jitter=True)
